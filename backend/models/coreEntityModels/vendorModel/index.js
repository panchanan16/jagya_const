...
class VendorModel {
   ...
   static async getVendor_Purcheses_Payments(vendor_id) {
      const query = `SELECT vp.*, p.pro_name,c.client_name FROM vendor_payments vp LEFT JOIN projects p ON p.pro_id = vp.pay_project_id LEFT JOIN clients c ON c.client_id=p.pro_client_r_id WHERE pay_vendor_id=?;
