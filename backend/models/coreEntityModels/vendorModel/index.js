...
class VendorModel {
   ...
   static async getVendor_Purcheses_Payments(vendor_id) {
      const query = `SELECT * FROM vendor_payments WHERE pay_vendor_id=?;
                     SELECT v.vendor_name,p.pro_name,c.client_name, mil.vendor_id ,mil.mr_item_id, mil.mr_project_r_id, mil.mr_item_name, mil.mr_item_quantity, mil.mr_item_amount, mil.mr_item_date FROM material_item_list mil LEFT JOIN vendors v ON v.vendor_id=mil.vendor_id LEFT JOIN projects p on p.pro_id= mil.mr_project_r_id LEFT JOIN clients c ON c.client_id=p.pro_client_r_id WHERE vendor_id=? AND fd_approval=1;
